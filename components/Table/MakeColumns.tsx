...
import doubleArrow from 'public/icons/Close.svg';
...
import { OldColumnType, makeColumnsTypes } from './Table.type';
...
const StepComponent = ({
  ...
  <div className="flex justify-center items-center flex-col">
    ...
    <p className="mt-3 text-12 whitespace-nowrap">
...
export const makeColumns: ({
  onClick,
}: makeColumnsTypes) => OldColumnType[] = ({ onClick }) => [
  {
    title: 'Request ID',
    key: 'requestId',
  ...
  },
  {
    ...
    key: 'status',
    render: (text) => (
      <p className={`text-${text}`}>{CapitalizeFirstLetter(text)}</p>
    ),
  },
  {
    title: '',
    ...
    render: (_, item: any) => (
      <button
        ...
        className="w-full border !border-primary rounded-md font-bold text-28 text-primary px-14 py-2.5 lg:w-fit">
        Detail
      </button>
    ),
  },
