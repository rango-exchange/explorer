  ...
  <li className="lg:mb-2 mb-[18px]">
    <div className="flex items-center">
      ...
      <button className={`ml-2 ${className || ''}`} onClick={onClick}>
        {value}{' '}
        {chain && (
          <span className="hidden text-neutral-600 lg:inline-block">
            ({chain})
          </span>
        )}
      </button>
    ...
    <div
      ...
      <div
        ...
        className={`lg:hidden mt-3 ${
          isOpen
            ? `border rounded-md mt-0 px-2.5 py-1.5 bg-neutral-100 absolute z-10 -top-4 left-5 right-5 border-${step.status}`
            : ''
        } cursor-pointer`}>
        <div className="flex justify-between items-center">
          <p className="text-16 font-bold">Step {index + 1}:</p>
      ...
      <div
        ...
        <div
          ...
          className={`flex items-center justify-between ${
            isOpen
              ? 'cursor-pointer lg:bg-neutral-100 lg:absolute lg:-top-6 lg:left-2 mt-3 lg:mt-0'
              : ''
          }`}>
          {!isOpen && (
            <div
              className={`w-6 h-6 rounded-full absolute -left-3 bg-${step.status}`}
            />
          )}
          {isOpen && (
            <div
              className={`w-14 ml-2 h-14 hidden border rounded-full mr-4 justify-center items-center text-16 font-semibold lg:flex border-${step.status} bg-${step.status} !bg-opacity-25`}>
              Step {index + 1}
            </div>
          )}
          <p className="text-16 font-normal lg:text-base lg:font-bold">
            <span
              className={`hidden ${isOpen ? 'lg:hidden' : 'lg:inline-block'}`}>
              Step {index + 1}:
            </span>{' '}
            From <span className="font-bold">{step.fromAsset.symbol}</span> (On{' '}
            {step.fromAsset.blockchain}) To{' '}
            <span className="font-bold">{step.toAsset.symbol}</span> (On{' '}
            {step.toAsset.blockchain}) Via {step.swapperId}
          </p>
        ...
        {isOpen && (
          <div className="mt-3 lg:mt-14">
            <p className="font-normal text-12 lg:font-bold lg:text-base">
              {step.fromAmount}{' '}
              <span className="font-bold">{step.fromAsset.symbol}</span>{' '}
              <span className="lg:text-neutral-600">
                (On {step.fromAsset.blockchain})
              </span>{' '}
              To {step.toAmount}{' '}
              <span className="font-bold">{step.toAsset.symbol}</span>{' '}
              <span className="lg:text-neutral-600">
                ({step.toAsset.blockchain})
              </span>
            </p>
            <ul className="font-bold list-disc ml-4 md:ml-10 text-12 lg:text-base lg:font-semibold mt-3">
              <Detail
            ...
            <div className="mt-5 lg:mt-10">
              ...
              <ul className="font-medium list-disc ml-4 md:ml-10 text-12 lg:text-base lg:font-bold mt-2 lg:mt-5 lg:w-2/4">
            ...
            <div>
              {/* <div className="flex items-center lg:mt-7 mt-4">
                  ...
                  <p className="text-12 lg:text-base lg:font-bold">
              ...
              {step.generatedTxId.length ? (
                <div className="mt-5 pt-5 border-t border-t-neutral-600">
                  {/* <p className="text-12 font-medium lg:font-bold lg:text-base">
                    Details: Your Swap Failed Because The Anysawp Needs At Least
                    1000 USD Coin To Perform The Bridging
                  </p> */}
                  <p className="font-bold text-12 lg:text-base">
