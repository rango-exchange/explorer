...
const Table: React.FC<PropsType> = ({ makeColumns, data, onClick }: PropsType) => {
  const columns: columns[] = useMemo(() => makeColumns({ onClick }), [makeColumns, onClick])
        ...
        {data.map((item, key) => (
          ...
          >
            {columns.map((row) => (
              ...
              >
                {!row.hiddenTitle && (
                  <div className="block lg:hidden font-normal text-neutral-900">{row.title}</div>
                )}
